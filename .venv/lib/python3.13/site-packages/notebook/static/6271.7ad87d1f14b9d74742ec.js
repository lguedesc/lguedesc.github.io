...
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([
  [6271],
  {
    /***/ 76271: /***/ (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      // ESM COMPAT FLAG
        ...
        toTree(nodeSet, end = this.end) {
          ...
          return new common_dist_index_js_.Tree(nodeSet.types[this.type], this.children, this.positions, end - this.from).balance({
            makeTree: (children, positions, length) =>
              new common_dist_index_js_.Tree(common_dist_index_js_.NodeType.none, children, positions, length, this.hashProp),
          });
        }
      }
      var Type;
      ...
      function skipForList(bl, cx, line) {
        ...
        return (
          size > 0 && (bl.type != Type.BulletList || isHorizontalRule(line, cx, false) < 0) && line.text.charCodeAt(line.pos + size - 1) == bl.value
        );
      }
      ...
      class LinkReferenceParser {
        ...
        finish(cx, leaf) {
          if (
            (this.stage == 2 /* RefStage.Link */ || this.stage == 3) /* RefStage.Title */ &&
        ...
        configure(spec) {
          ...
          if (nonEmpty(config.defineNodes)) {
            ...
            for (let s of config.defineNodes) {
              ...
              if (style) {
                if (!styles) styles = {};
                if (Array.isArray(style) || style instanceof highlight_dist_index_js_.Tag) styles[name] = style;
                else Object.assign(styles, style);
              }
            }
            nodeSet = new common_dist_index_js_.NodeSet(nodeTypes);
            if (styles) nodeSet = nodeSet.extend((0, highlight_dist_index_js_.styleTags)(styles));
          }
      ...
      for (let i = 1, name; (name = Type[i]); i++) {
        nodeTypes[i] = common_dist_index_js_.NodeType.define({
          ...
          props:
            i >= Type.Escape
              ? []
              : [[common_dist_index_js_.NodeProp.group, i in DefaultSkipMarkup ? ["Block", "BlockContext"] : ["Block", "LeafBlock"]]],
          top: name == "Document",
        });
      }
      const none = [];
      ...
      const DefaultInline = {
          ...
          for (; pos < cx.end; pos++) {
            if (cx.char(pos) == 96) {
              ...
              if (curSize == size && cx.char(pos + 1) != 96)
                return cx.append(
                  elt(Type.InlineCode, start, pos + 1, [elt(Type.CodeMark, start, start + size), elt(Type.CodeMark, pos + 1 - size, pos + 1)])
                );
            } else {
              curSize = 0;
            }
        ...
        Emphasis(cx, next, start) {
          ...
          return cx.append(
            new InlineDelimiter(
              ...
              start,
              pos,
              (canOpen ? 1 /* Mark.Open */ : 0) /* Mark.None */ | (canClose ? 2 /* Mark.Close */ : 0) /* Mark.None */
            )
          );
        },
        ...
        Image(cx, next, start) {
          return next == 33 /* '!' */ && cx.char(start + 1) == 91 /* '[' */
            ? cx.append(new InlineDelimiter(ImageStart, start, start + 2, 1 /* Mark.Open */))
            : -1;
        },
      ...
      class InlineContext {
        ...
        addDelimiter(type, from, to, open, close) {
          return this.append(
            new InlineDelimiter(type, from, to, (open ? 1 /* Mark.Open */ : 0) /* Mark.None */ | (close ? 2 /* Mark.Close */ : 0) /* Mark.None */)
          );
        }
          ...
          for (let i = from; i < this.parts.length; i++) {
            ...
            for (; j >= from; j--) {
              ...
              if (
                ...
                !(
                  emp &&
                  (close.side & 1 /* Mark.Open */ || part.side & 2) /* Mark.Close */ &&
      ...
      const parser = new MarkdownParser(
        new common_dist_index_js_.NodeSet(nodeTypes).extend(markdownHighlighting),
      ...
      function parseRow(cx, line, startI = 0, elts, offset = 0) {
        ...
        let parseCell = () => {
          elts.push(
            cx.elt("TableCell", offset + cellStart, offset + cellEnd, cx.parser.parseInline(line.slice(cellStart, cellEnd), offset + cellStart))
          );
        };
...
*/
      ...
      function getCodeParser(languages, defaultLanguage) {
        return (info) => {
          if (info && languages) {
            ...
            else found = language_dist_index_js_.LanguageDescription.matchLanguageName(languages, info, true);
      ...
      function normalizeIndent(content, state) {
        ...
        if (!blank || state.facet(language_dist_index_js_.indentUnit) != "\t") return content;
          ...
          changes = state.changeByRange((range) => {
            ...
            if (inner.item && emptyLine) {
              ...
              if (first.to >= pos || (second && second.to < pos) || (line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text))) {
                ...
                return {
                  range: index_js_.EditorSelection.cursor(pos + insert.length + 1),
      ...
      function blankLine(context, state, line) {
        ...
        for (let i = 0, e = context.length - 2; i <= e; i++) {
          insert += context[i].blank(
            i < e ? (0, index_js_.countColumn)(line.text, 4, Math.min(line.text.length, context[i + 1].from)) - insert.length : null,
            i < e
          );
        }
            ...
            if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
              ...
              if (context.length) {
                ...
                if (pos - line.from > spaceEnd && !/\S/.test(line.text.slice(spaceEnd, pos - line.from)))
                  return { range: index_js_.EditorSelection.cursor(line.from + spaceEnd), changes: { from: line.from + spaceEnd, to: pos } };
                if (
                  ...
                  if (inner.item && inner.node.from < inner.item.from && /\S/.test(line.text.slice(inner.from, inner.to))) {
                    let insert = inner.blank(
                      (0, index_js_.countColumn)(line.text, 4, inner.to) - (0, index_js_.countColumn)(line.text, 4, inner.from)
                    );
                    if (start == line.from) insert = normalizeIndent(insert, state);
                    return {
                      range: index_js_.EditorSelection.cursor(start + insert.length),
                      changes: { from: start, to: line.from + inner.to, insert },
                    };
                  }
                  // Delete one level of indentation
                  if (start < pos) return { range: index_js_.EditorSelection.cursor(start), changes: { from: start, to: pos } };
                }
              }
            }
            return (dont = { range });
...
*/
      function markdown(config = {}) {
        ...
        if (addKeymap) support.push(index_js_.Prec.high(dist_index_js_.keymap.of(markdownKeymap)));
      ...
      function htmlTagCompletions() {
        ...
        let result = (0, lang_html_dist.htmlCompletionSource)(
          new dist /* CompletionContext */.TK(index_js_.EditorState.create({ extensions: htmlNoMatch }), 0, true)
        );
