    ...
    /***/ 22955: /***/ (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      ...
      class ContentView {
        ...
        toString() {
          ...
          return (
            name +
            (this.children.length ? "(" + this.children.join() + ")" : this.length ? "[" + (name == "Text" ? this.text : this.length) + "]" : "") +
            (this.breakAfter ? "#" : "")
          );
        }
        static get(node) {
      ...
      class MarkView extends ContentView {
        ...
        merge(from, to, source, _hasStart, openStart, openEnd) {
          if (
            source &&
            (!(source instanceof MarkView && source.mark.eq(this.mark)) || (from && openStart <= 0) || (to < this.length && openEnd <= 0))
          )
            return false;
    ...
    */
      ...
      class DocView extends ContentView {
        ...
        fixCompositionDOM(composition) {
          let fix = (dom, cView) => {
            cView.flags |=
              8 /* ViewFlag.Composition */ | (cView.children.some((c) => c.flags & 7 /* ViewFlag.Dirty */) ? 1 /* ViewFlag.ChildDirty */ : 0);
      ...
      function groupAt(state, pos, bias = 1) {
        ...
        let from = linePos,
          to = linePos;
        if (bias < 0) from = (0, index_js_.findClusterBreak)(line.text, linePos, false);
      ...
      function moveToLineBoundary(view, start, forward, includeWrap) {
        ...
        if (coords) {
          ...
          if (pos != null) return index_js_.EditorSelection.cursor(pos, forward ? -1 : 1);
        }
      ...
      function moveVertically(view, start, forward, distance) {
        let startPos = start.head,
          dir = forward ? 1 : -1;
        if (startPos == (forward ? view.state.doc.length : 0)) return index_js_.EditorSelection.cursor(startPos, start.assoc);
      ...
      class DOMChange {
        constructor(view, start, end, typeOver) {
          ...
          if (view.state.readOnly && start > -1) {
            ...
            let anchor =
              (iAnchor && iAnchor.node == domSel.anchorNode && iAnchor.offset == domSel.anchorOffset) || !contains(view.contentDOM, domSel.anchorNode)
      ...
      function applyDOMChange(view, domChange) {
        ...
        if (!change && domChange.typeOver && !sel.empty && newSel && newSel.main.empty) {
          ...
          if (newSel && change.insert.length == 2) newSel = index_js_.EditorSelection.single(newSel.main.anchor - 1, newSel.main.head - 1);
          ...
          if (newSel) newSel = index_js_.EditorSelection.single(newSel.main.anchor - 1, newSel.main.head - 1);
        ...
        skipAtoms(sel) {
          ...
          for (let i = 0; i < sel.ranges.length; i++) {
            ...
            if (range.empty) {
              ...
              if (pos != range.from) updated = index_js_.EditorSelection.cursor(pos, -1);
            } else {
      ...
      function doPaste(view, input) {
        ...
        if (linewise) {
          ...
          changes = state.changeByRange((range) => {
            ...
            return { changes: { from: line.from, insert }, range: index_js_.EditorSelection.cursor(range.from + insert.length) };
          });
        } else if (byLine) {
          changes = state.changeByRange((range) => {
      ...
      handlers.dragstart = (view, event) => {
        ...
        if (event.target.draggable) {
          ...
          if (cView && cView.isWidget) {
            ...
            if (from >= range.to || to <= range.from) range = index_js_.EditorSelection.range(from, to);
          }
        }
        let { inputState } = view;
    ...
    */
      ...
      class ViewState {
        constructor(state) {
          ...
          this.heightMap = HeightMap.empty().applyChanges(this.stateDeco, index_js_.Text.empty, this.heightOracle.setDoc(state.doc), [
        ...
        updateScaler() {
          ...
          this.scaler =
            this.heightMap.height <= 7000000 /* VP.MaxDOMHeight */ ? IdScaler : new BigScaler(this.heightOracle, this.heightMap, this.viewports);
        ...
        update(update, scrollTarget = null) {
          ...
          let heightChanges = ChangedRange.extendWithRanges(
            contentChanges,
            heightRelevantDecoChanges(prevDeco, this.stateDeco, update ? update.changes : index_js_.ChangeSet.empty(this.state.doc.length))
          );
        ...
        measure(view) {
          ...
          if (measureContent) {
            ...
            for (let vp of this.viewports) {
              ...
              this.heightMap = (
                refresh
                  ? HeightMap.empty().applyChanges(this.stateDeco, index_js_.Text.empty, this.heightOracle, [
        ...
        getViewport(bias, scrollTarget) {
          ...
          let viewport = new Viewport(
            map.lineAt(visibleTop - marginTop * 1000 /* VP.Margin */, QueryType.ByHeight, oracle, 0, 0).from,
            map.lineAt(visibleBottom + (1 - marginTop) * 1000 /* VP.Margin */, QueryType.ByHeight, oracle, 0, 0).to
          );
          ...
          if (scrollTarget) {
            ...
            if (head < viewport.from || head > viewport.to) {
              ...
              viewport = new Viewport(
                map.lineAt(topPos - 1000 /* VP.Margin */ / 2, QueryType.ByHeight, oracle, 0, 0).from,
                map.lineAt(topPos + viewHeight + 1000 /* VP.Margin */ / 2, QueryType.ByHeight, oracle, 0, 0).to
              );
            }
          }
          return viewport;
        ...
        viewportIsAppropriate({ from, to }, bias = 0) {
          ...
          let { visibleTop, visibleBottom } = this;
          return (
            (from == 0 || top <= visibleTop - Math.max(10 /* VP.MinCoverMargin */, Math.min(-bias, 250 /* VP.MaxCoverMargin */))) &&
        ...
        lineBlockAt(pos) {
          return (
            (pos >= this.viewport.from && pos <= this.viewport.to && this.viewportLines.find((b) => b.from <= pos && b.to >= pos)) ||
      ...
      class DOMObserver {
        ...
        readSelectionRange() {
          ...
          let range =
            (browser.safari && view.root.nodeType == 11 && view.root.activeElement == this.dom && safariSelectionRangeHack(this.view, selection)) ||
            selection;
          ...
          this.handlers.textupdate = (e) => {
            ...
            if (!view.state.readOnly) {
              ...
              applyDOMChangeInner(
                view,
                change,
                index_js_.EditorSelection.single(this.toEditorPos(e.selectionStart, newLen), this.toEditorPos(e.selectionEnd, newLen))
              );
            }
          ...
          this.handlers.textformatupdate = (e) => {
            ...
            for (let format of e.getTextFormats()) {
              ...
              if (lineStyle != "None" && thickness != "None") {
                ...
                if (from < to) {
                  let style = `text-decoration: underline ${lineStyle == "Dashed" ? "dashed " : lineStyle == "Squiggle" ? "wavy " : ""}${
                    thickness == "Thin" ? 1 : 2
                  }px`;
...
*/
    ...
    */
        ...
        */
          ...
          this.dispatchTransactions =
            config.dispatchTransactions || (dispatch && ((trs) => trs.forEach((tr) => dispatch(tr, this)))) || ((trs) => this.update(trs));
    ...
    */
        update(transactions) {
          ...
          if (state.facet(index_js_.EditorState.phrases) != this.state.facet(index_js_.EditorState.phrases)) return this.setState(state);
        ...
        get hasFocus() {
          ...
          return (
            (this.dom.ownerDocument.hasFocus() ||
              (browser.safari && ((_a = this.inputState) === null || _a === void 0 ? void 0 : _a.lastContextMenu) > Date.now() - 3e4)) &&
    ...
    */
        static scrollIntoView(pos, options = {}) {
          return scrollIntoView.of(
            new ScrollTarget(
              typeof pos == "number" ? index_js_.EditorSelection.cursor(pos) : pos,
              options.y,
              options.x,
              options.yMargin,
              options.xMargin
    ...
    */
        scrollSnapshot() {
          ...
          return scrollIntoView.of(
            new ScrollTarget(index_js_.EditorSelection.cursor(ref.from), "start", "start", ref.top - scrollTop, scrollLeft, true)
          );
        }
        /**
        ...
        static update(cache, changes) {
          ...
          for (let i = Math.max(0, cache.length - 10); i < cache.length; i++) {
            ...
            if (entry.dir == lastDir && !changes.touchesRange(entry.from, entry.to))
              result.push(
                new CachedOrder(changes.mapPos(entry.from, 1), changes.mapPos(entry.to, -1), entry.dir, entry.isolates, false, entry.order)
              );
          }
          return result;
        }
...
*/
      ...
      function runHandlers(map, event, view, scope) {
        ...
        let charCode = (0, index_js_.codePointAt)(name, 0),
          isChar = (0, index_js_.codePointSize)(charCode) == name.length && name != " ";
        let prefix = "",
          handled = false,
          prevented = false,
          stopPropagation = false;
                  ...
                  decoration: (m, view, pos) => {
                    ...
                    if (code == 9) {
                      ...
                      let size = view.state.tabSize,
                        col = (0, index_js_.countColumn)(line.text, size, pos - line.from);
                      return Decoration.replace({
                        widget: new TabWidget(((size - (col % size)) * this.view.defaultCharacterWidth) / this.view.scaleX),
                      });
                    }
                    return (
                      this.decorationCache[code] || (this.decorationCache[code] = Decoration.replace({ widget: new SpecialCharWidget(conf, code) }))
                    );
                  },
...
*/
      ...
      class Placeholder extends WidgetType {
        ...
        coordsAt(dom) {
          ...
          if (rect.bottom - rect.top > lineHeight * 1.5) return { left: rect.left, right: rect.right, top: rect.top, bottom: rect.top + lineHeight };
          return rect;
        }
        ignoreEvent() {
          return false;
      ...
      function rectangleFor(state, a, b) {
        ...
        if (a.off > MaxOff || b.off > MaxOff || a.col < 0 || b.col < 0) {
          ...
          for (let i = startLine; i <= endLine; i++) {
            ...
            if (line.length <= endOff) ranges.push(index_js_.EditorSelection.range(line.from + startOff, line.to + endOff));
          }
        } else {
      ...
      function rectangleSelectionStyle(view, event) {
        ...
        return {
          ...
          get(event, _extend, multiple) {
            ...
            if (multiple) return index_js_.EditorSelection.create(ranges.concat(startSel.ranges));
            ...
            for (let i = 0; i < this.manager.tooltips.length; i++) {
              ...
              let left =
                size.width > space.right - space.left
                  ...
                  : ltr
                    ? Math.max(space.left, Math.min(pos.left - (arrow ? 14 /* Arrow.Offset */ : 0) + offset.x, space.right - width))
                    : Math.min(Math.max(space.left, pos.left - width + (arrow ? 14 /* Arrow.Offset */ : 0) - offset.x), space.right - width);
              let above = this.above[i];
              if (
                !tooltip.strictSide &&
                (above ? pos.top - height - arrowHeight - offset.y < space.top : pos.bottom + height + arrowHeight + offset.y > space.bottom) &&
