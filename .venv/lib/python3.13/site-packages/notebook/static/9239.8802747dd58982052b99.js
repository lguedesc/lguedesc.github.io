...
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([
  [9239],
  {
    /***/ 59239: /***/ (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      // ESM COMPAT FLAG
   ...
   expressed by Lezer's built-in tokenizer. */
      ...
      const parser = dist /* LRParser */.WQ.deserialize({
        ...
        tokenizers: [
          ...
          3,
          4,
          new dist /* LocalTokenGroup */.RA("m~RRYZ[z{a~~g~aO#^~~dP!P!Qg~lO#_~~", 28, 105),
        ],
      ...
      function variableNames(doc, node, isVariable) {
        if (node.to - node.from > 4096) {
          ...
          let result = [],
            seen = new Set(),
            cursor = node.cursor(common_dist_index_js_.IterMode.IncludeAnonymous);
          ...
          node.cursor().iterate((node) => {
            ...
            if (
              ...
              ((_a = node.node.nextSibling) === null || _a === void 0 ? void 0 : _a.name) == ":"
            ) {
...
*/
      const defineCSSCompletionSource = (isVariable) => (context) => {
        ...
        if (node.name == "ValueName") return { from: node.from, options: values, validFor: dist_identifier };
        if (node.name == "PseudoClassName") return { from: node.from, options: pseudoClasses, validFor: dist_identifier };
        ...
        if (node.name == "TagName") {
          for (let { parent } = node; parent; parent = parent.parent)
            if (parent.name == "Block") return { from: node.from, options: properties(), validFor: dist_identifier };
          return { from: node.from, options: tags, validFor: dist_identifier };
        }
        if (node.name == "AtKeyword") return { from: node.from, options: atRules, validFor: dist_identifier };
        ...
        if (above.name == "Block" || above.name == "Styles") return { from: pos, options: properties(), validFor: dist_identifier };
        return null;
      };
      /**
